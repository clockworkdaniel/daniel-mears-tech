import {
  TweenMax, TimelineLite, Power1, Power2, Power3, Bounce
} from 'gsap/TweenMax';
import DrawSVPGPlugin from '../vendor/DrawSVGPlugin'; // eslint-disable-line
...
export default function introAnimation() {
  ...
  const headerTitles = 'header h1';
  const navElements = 'header li, .header__nav-trigger-container';
  const casinoElements = '.casino path, .casino circle, .casino polyline, .casino rect';

  const intro = new TimelineLite();

  intro.set('.casino', { opacity: 0.6 });
  intro.set(casinoElements, {
    stroke: '#FFFFFF', strokeMiterlimit: 10, opacity: 0, fillOpacity: 0
  });
  ...
  if (viewportSize.width > 900) {
    intro.add(TweenMax.to('.casino', 1.9, {
      scale: 3, opacity: 0.3, x: '121%', y: '-46%', rotation: '-11deg', ease: Power2.easeInOut
    }), '-=2');
    intro.set('.header__title', { opacity: 1 }, '-=0.8');
    intro.set('.header__subtitle', { opacity: 1 }, '+=0.5');
  } else {
    intro.add(TweenMax.to('.casino', 1.9, {
      scale: 7.4, opacity: 0.3, x: '42%', y: '-328%', rotation: '-71deg', ease: Power2.easeInOut
    }), '-=2');
